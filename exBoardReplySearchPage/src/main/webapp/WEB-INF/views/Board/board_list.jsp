	...
	<c:forEach var="board" items="${list}">
		<tr onMouseOver="style.backgroundColor='#D1EEEE'" onMouseOut="style.backgroundColor=''">
			...
			<td align="left" height="20">&nbsp;
				...
				<a class="list" href="board_view?page=${nowpage}&idx=${board.idx}">${board.subject}</a></td>
	...
	<table width="600">
		<tr>
			...
			<td width="50%" align="center">
				<table>
					<form name="b_search" method="post" action="board_list">	
					<input type="hidden" name="page" value="${nowpage}">
					<!-- 검색어를 이용하여 글제목, 작성자, 글내용 중에 하나를 입력 받아 처리하기 위한 부분 -->
						<tr>
							<td>
								<select name="search">
									<option value="subject" <c:if test="${pageVO.search=='subject'}">selected</c:if>>글제목</option>
									<option value="name" <c:if test="${pageVO.search=='name'}">selected</c:if>>작성자</option>
									<option value="contents" <c:if test="${pageVO.search=='contents'}">selected</c:if>>글내용</option>
								</select>
							</td>
							<td> <input type="text" size=20 name="key" value="${pageVO.key}"></td>
							<td> <a href="javascript:b_sCh()"><img src="/img/search2.gif" border="0"></a></td>
						</tr>
					</form>
				</table>
			</td>
			<td width="25%" align="right">
			<a href="board_write?page=${nowpage}"><img src="/img/write.gif" border="0"></a>
			</td>
		</tr>
	</table>
</body>
